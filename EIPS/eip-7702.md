...
For the duration of the transaction, instructions that access the storage of any of the above recovered `signer` accounts MUST cause execution to become invalid (in the same way as a stack underflow or invalid jump.) Examples of such instructions include `SSTORE` (`0x55`) / `SLOAD` (`0x54`) in the context of a `signer`, and any of the same in a `delegatecall`/`callcode` target from a `signer`.
...
### Prohibiting Storage Access

#### Stores

There are two primary concerns with storage in EOAs and this proposal: collisions and poisoning.

Because users may sign any `contract_code` values and will likely sign several different ones, there is a high risk of colliding storage slots. Having to coordinate and track which bytecodes have been signed over is impossible to track automatically (different wallet software, unpublished signatures, etc.) and is a large burden for users to do by hand.

The second, and more insidious, attack is a specially crafted `contract_code` setting a seemingly innocent storage slot that—when the EOA is eventually upgraded to a smart contract wallet—collides with a slot in a popular smart wallet implementation, effectively adding a hidden signer to the account.

There are two obvious solutions to these problems: prohibit writing to storage, and resetting storage at the end of the transaction (i.e. making `SSTORE` behave like `TSTORE`.) This proposal chooses to go with prohibiting `SSTORE` because it fails safely. Imagine a preexisting smart contract wallet implementation that uses storage to track its replay protection nonce. If all `SSTORE` transparently behaved like `TSTORE`, it would _appear_ to work when deployed to an EOA, but after the transaction is finished the storage would be reset, enabling replay attacks.

#### Loads

While less problematic than stores, reading from storage can hide unexpected behavior. Considering that changing any storage slot is prohibited, reads must always return zero, and any attempt to do so is very suspect.
